...
def _traceback_randomwalk(DTW_length, window, prob, mode = 'static', alpha = 2):
    '''
    mode
    static: probability does not change
    dynamic: probability changes by path hisdtory
    '''
    ...
    if mode == 'dynamic':
        prob = [alpha, alpha, alpha]
        cnt = 0



    while (i > 1):
        # check if the condition is out of the window
        
        if mode == 'static':
            start = max(1, i-window)
            end = min(DTW_length, i+window)+1
            
            if (j > start) and (j < end):
                # within window
                tb = np.random.choice(3, p = prob)
                if (tb == 0):
                    i = i - 1
                    j = j - 1
                elif (tb == 1):
            ...
            tb = np.random.choice(3, p = prob/np.array(prob).sum(0))
            ...
            # modify the prob based on cnt
            if cnt < 0:
                prob = [alpha + abs(cnt), alpha + abs(cnt), alpha]
            else:     
                prob = [alpha+ abs(cnt), alpha , alpha + abs(cnt)]                
            
            
        p.insert(0, i-1)
        q.insert(0, j-1)
        
    assert len(np.unique(np.array(p))) == DTW_length
    return np.array(q)
...
def _traceback(DTW, slope_constraint):
    i, j = np.array(DTW.shape) - 1 # 150, 150
    p, q = [i-1], [j-1] # [149], [149]
    ...
    # assert (len(np.unique(p)) == DTW.shape[0]) and (len(np.unique(q)) == DTW.shape[1])
