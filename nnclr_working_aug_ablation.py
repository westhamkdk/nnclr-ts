...
from utils_dtw import data_augmentation as da

LABEL_UNDEFINED = -1        


class MyTransformedDataset(Dataset):
    def __init__(self, train_data, train_labels, label_mask, transform=None):
        self.train_data = torch.from_numpy(train_data).float()
        self.train_labels = torch.from_numpy(train_labels).float()
        self.label_mask = torch.from_numpy(label_mask).float()
        self.transform = transform
        
    def __getitem__(self, index):
        x_org = self.train_data[index]
        y = self.train_labels[index]

        # Apply the transformations
        x_aug1 = self.transform(x_org) if self.transform else x_org.clone()
        x_aug2 = self.transform(x_org) if self.transform else x_org.clone()

        # Ensure they are float32 type
        x_aug1 = x_aug1.float() if isinstance(x_aug1, torch.Tensor) else torch.from_numpy(x_aug1).float()
        x_aug2 = x_aug2.float() if isinstance(x_aug2, torch.Tensor) else torch.from_numpy(x_aug2).float()

        label_mask = self.label_mask[index]

        return x_org, x_aug1, x_aug2, y, label_mask

    def __len__(self):
        return len(self.train_data)


# original_labels와 label_mask를 유념하도록    
class CustomNets:
...
class NNCLROtherAugs:
    ...
    def fit(self, train_data, train_labels, n_epochs = None, n_iters = None, verbose = False):
        ...
        '''
        # shuffle only once
        train_dataset = MyTransformedDataset(train_data, train_labels, label_mask_org, self.other_aug)            
        ...
        while True:
            ...
            for b_index, batch in enumerate(train_loader):
                if batch[0].shape[0] >= 0:
                    ...
                    x_org, x_pos1_org, x_pos2_org, y_org, label_mask= batch[0] ,batch[1], batch[2], batch[3], batch[4]
